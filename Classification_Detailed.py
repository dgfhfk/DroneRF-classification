...
results_path = r"D:\code\drone\dronerf\output\results"  # 详细分类结果的保存路径
...
# 使用标签3（详细分类）
label_to_use = Label_3
y = encode(label_to_use)  # 将标签转换为独热编码
n_classes = y.shape[1]  # 类别数量

# 定义详细的类别名称
# 根据Main_2_Data_labeling.m文件中的BUI定义，创建详细的类别名称
class_names = [
    "Background",          # 0: 背景
    "Bebop_Mode1",         # 1: Bebop无人机模式1
    "Bebop_Mode2",         # 2: Bebop无人机模式2
    "Bebop_Mode3",         # 3: Bebop无人机模式3
    "Bebop_Mode4",         # 4: Bebop无人机模式4
    "AR_Mode1",            # 5: AR无人机模式1
    "AR_Mode2",            # 6: AR无人机模式2
    "AR_Mode3",            # 7: AR无人机模式3
    "AR_Mode4",            # 8: AR无人机模式4
    "Phantom"              # 9: Phantom无人机
]
