...
outer_activation_fun = 'sigmoid'  # 输出层激活函数，使用Sigmoid函数将输出压缩到0-1之间

# 优化器参数
optimizer_loss_fun   = 'mse'  # 损失函数，使用均方误差(Mean Squared Error)
optimizer_algorithm  = 'adam'  # 优化算法，使用Adam优化器，结合了动量和自适应学习率

# 训练参数
number_epoch         = 30  # 训练轮数，每个样本将被训练20次
batch_length         = 10  # 批量大小，每次更新权重使用10个样本
show_inter_results   = 1   # 是否显示中间结果，1表示显示训练过程的简要信息

# 文件路径设置
data_path = r"D:\code\dronerf\output\labeled_data\RF_Data.csv"  # 标记数据的路径（文件2的输出）
results_path = r"D:\code\dronerf\output\classification_results"  # 分类结果的保存路径
...
Label_1 = np.transpose(Data[2048:2049,:]); Label_1 = Label_1.astype(int)  # 二分类标签（背景/无人机）
Label_2 = np.transpose(Data[2049:2050,:]); Label_2 = Label_2.astype(int)  # 四分类标签（背景/Bebop/AR/Phantom）
Label_3 = np.transpose(Data[2050:2051,:]); Label_3 = Label_3.astype(int)  # 详细分类标签

# 选择要使用的标签（这里使用标签2，即四分类）
